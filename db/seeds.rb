...
10.times do
  ...
  5.times do
    ...
    p.save!
    r = Requisition.new(status: ["pendente", "aprovada", "reprovada"].sample, field_name: ["IPTU", "ITBI", "ISS", "Dados Escolares", "Dados de Saúde"].sample, new_value: Faker::Lorem.sentence(word_count: 5), justification: Faker::Lorem.sentence(word_count: 5), excluded: [false, true].sample, user: u, personal_datum: p)
    r.save!
  end
